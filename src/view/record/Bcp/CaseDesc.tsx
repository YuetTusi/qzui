        ...
        return <div className="sort">
            <div className="case-info">
                <Descriptions bordered={true} size="small">
                    <Item label="所属案件" span={3}><span>{caseData.m_strCaseName.split('_')[0]}</span></Item>
                    {/* <Descriptions.Item label="送检单位" span={3}>{caseData.m_strDstCheckUnitName}</Descriptions.Item> */}
                    <Item label="手机名称"><span>{deviceData?.mobileName ? deviceData?.mobileName!.split('_')[0] : ''}</span></Item>
                    <Item label="手机持有人"><span>{deviceData?.mobileHolder}</span></Item>
                    <Item label="手机编号">{deviceData?.mobileNo}</Item>
                    <Item label="取证时间">{moment(deviceData?.fetchTime).format('YYYY-MM-DD HH:mm:ss')}</Item>
