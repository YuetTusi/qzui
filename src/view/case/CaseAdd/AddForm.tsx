import React, { MouseEvent, memo, useRef, forwardRef } from 'react';
...
interface Context {
     ...
     * 采集人员Change事件
     */
    officerChange: (value: string, option: React.ReactElement<any> | React.ReactElement<any>[]) => void,
    /**
     * 绑定采集人员Options
     */
...
const AddForm = Form.create<Prop>()(forwardRef<Form, Prop>((props: Prop) => {

    const { getFieldDecorator } = props.form;
    const {
        historyUnitNames, chooiseApp, autoParse,
        generateBcp, disableGenerateBcp, attachment,
        disableAttachment, apps, context
    } = props;
    ...
    return <Form {...formItemLayout}>
            ...
            <Col span={24}>
                <Item label="检验单位">
                    {getFieldDecorator('checkUnitName', {
                        ...
                        ? []
                        : historyUnitNames.reduce((total: string[], current: string, index: number) => {
            ...
            <div className="ant-col ant-col-18 ant-form-item-control-wrapper">
                <div className="inner">
                    <Tooltip title="手动选择采集的应用">
                        <Checkbox onChange={context.chooiseAppChange} checked={chooiseApp} />
                    </Tooltip>
                    <span>自动解析：</span>
        ...
        <Item className="app-list-item">
            <div className="app-list-panel" style={{ display: chooiseApp ? 'block' : 'none' }}>
                <AppList apps={apps} />
            </div>
        </Item>
    </Form>;
