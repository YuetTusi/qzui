...
const ExtendFetchLog = Form.create<Prop>({ name: 'SearchForm' })(
	...
	class FetchLog extends Component<Prop, State> {
		...
		queryTable(condition: any = {}, current: number = 1, pageSize: number = 10) {
			const { dispatch } = this.props;
			dispatch({
				type: 'fetchLog/queryAllFetchLog',
				payload: {
					condition,
					current,
					pageSize
		...
		searchFormSubmit = () => {
			...
			this.queryTable(
				{
					start: helper.isNullOrUndefined(start)
						? start
						: (start as Moment).format('YYYY-MM-DD HH:mm:ss'),
					end: helper.isNullOrUndefined(end)
						? end
						: (end as Moment).format('YYYY-MM-DD HH:mm:ss')
				},
				1,
				10
			);
				...
				<div className="search-bar">
					...
					<div className="fn">
						...
						<HiddenToggle show={this.isAdmin}>
							<ModeButton type="danger" onClick={() => this.dropAllDataHandle()}>
								<Icon type="delete" />
								<span>全部清除</span>
							</ModeButton>
						</HiddenToggle>
		...
		render(): JSX.Element {
			...
			return (
				<div className="fetch-log-root">
					<div className="search-form">{this.renderForm()}</div>
					<div className="scroll-panel">
						<div className="table-panel">{this.renderTable(data, total)}</div>
					</div>
					<DelLogModal
