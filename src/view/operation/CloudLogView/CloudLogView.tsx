 ...
 * @param props
 */
const CloudLogView = Form.create({ name: 'cloudLogForm' })(
	({ dispatch, cloudLog, form, location }: Prop) => {
		const { loading, data, pageSize, current, total, detailVisible, apps } = cloudLog;
		const [delModalVisible, setDelModalVisible] = useState(false);
		const isAdmin = useRef(false);

		useMount(() => {
			const [, roleName] = location.search.split('=');
		...
		const pageChange = (current: number, pageSize?: number) => {
			let condition = form.getFieldsValue();
			dispatch({
				type: 'cloudLog/queryCloudLog',
				payload: {
					condition,
		...
		const renderTable = (): JSX.Element => {
			return (
				<Table<CloudLog>
					dataSource={data}
					columns={getColumns(dispatch, isAdmin.current)}
					loading={loading}
					bordered={true}
