	...
	const updateBcpConfResultHandle = (event: IpcRendererEvent, result: Record<string, any>) => {
		...
		if (result.success) {
			...
			localStorage.setItem(
				'manufacturer',
				helper.isNullOrUndefinedOrEmptyString(current?.manufacturer)
					? ''
					: current?.manufacturer!
			);
			localStorage.setItem(
				'security_software_orgcode',
				helper.isNullOrUndefinedOrEmptyString(current?.securitySoftwareOrgCode)
					? ''
					: current?.securitySoftwareOrgCode!
			);
			localStorage.setItem(
				'materials_name',
				helper.isNullOrUndefinedOrEmptyString(current?.materialsName)
					? ''
					: current?.materialsName!
			);
			localStorage.setItem(
				'materials_model',
				helper.isNullOrUndefinedOrEmptyString(current?.materialsModel)
					? ''
					: current?.materialsModel!
			);
			localStorage.setItem(
				'materials_hardware_version',
				helper.isNullOrUndefinedOrEmptyString(current?.materialsHardwareVersion)
					? ''
					: current?.materialsHardwareVersion!
			);
			localStorage.setItem(
				'materials_software_version',
				helper.isNullOrUndefinedOrEmptyString(current?.materialsSoftwareVersion)
					? ''
					: current?.materialsSoftwareVersion!
			);
			localStorage.setItem(
				'materials_serial',
				helper.isNullOrUndefinedOrEmptyString(current?.materialsSerial)
					? ''
					: current?.materialsSerial!
			);
			localStorage.setItem(
				'ip_address',
				helper.isNullOrUndefinedOrEmptyString(current?.ipAddress) ? '' : current?.ipAddress!
			);
		} else {
			message.error('保存失败');
		}
