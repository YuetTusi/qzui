...
import React, { memo, ReactElement, FC, MouseEvent, useState, useRef } from 'react';
...
const ImportDataModal: FC<Prop> = (props) => {
	...
	const bindCaseSelect = () => {
		...
		return caseList.map((opt: CCaseInfo, i) => {
			...
			return (
				<Option value={opt.m_strCaseName} data-case-path={opt.m_strCasePath} key={`C_${i}`}>
	...
	const bindOfficerSelect = () => {
		...
		return officerList.map((opt) => {
			return (
				<Option value={opt.no} data-name={opt.name} data-id={opt.no} key={helper.getKey()}>
					{`${opt.name}（${opt.no}）`}
				</Option>
			);
		});
	...
	const caseChange = (val: string, opt: any) => {
		const casePath = opt.props['data-case-path'];
		console.log(path.join(casePath, val));
	};
