 ...
 * 第三方平台RPC对象
 */
let platformer: (Rpc | null) = null;
if (config.usePlatformData && platformer === null) {
    //NOTE：配置文件中指定了从第三方平台获取案件数据，则创建platformer的RPC实例
    platformer = new Rpc(config.platformUri);
}

//NOTE: 采集和解析RPC对象整个应用全局唯一，且不可更改（对象为const常量）
//NOTE: 在发布反向接口时，只在启动应用时监听一次

export { fetcher, parser, platformer };
export default Rpc;