...
> 当 mode===3 云检测，由 Fetch 推送应用列表，根据应用结果状态来以颜色区分检测成功与失败
...
| ------------- | ----------- | --------------------------------------------- |
| usb           | number      | USB 序号                                      |
| mode          | enum        | 0:标准采集,1:点验,2:广州警综平台,3:短信云检测 |
...
| tokenAppList | string[] | Token 云检测应用包名                                  |
| dataMode     | enum     | 模式（0：标准,1：点验,2：广州警综平台,3：短信云检测） |
